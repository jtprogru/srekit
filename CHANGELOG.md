...
- **`srekit tasker` — a task card for a collection of engineering tasks.** Front matter (`type: simple_note`, `tags: [tasker]`, `topic`, `level`, `format`, `duration`), an H1 reading `Tasker - <name>`, and two sections — `description` and `expectations` — both empty. Empty on purpose: the content is written by the person adding the task, and a placeholder would be text they delete on every single card. Flags: `--title` (required), `--topic` (default `go`), `--level` (repeatable or comma-separated, default `middle,senior`), `--format` (default `code`), `--duration` in minutes (default `30`). Default filename `tasker-<slug>.md`. This widens the catalog past "artifacts an on-call engineer owns": it is the first shipped document that describes a task for another engineer rather than work on production. `srekit task` (the investigation log) is untouched — similar names, unrelated documents.
- **Non-string front matter values in v1 artifacts.** An explicit YAML tag on a value (`!!int`, `!!float`, `!!bool`, `!!null`, `!!timestamp`, `!!seq`, `!!map`) tells the renderer to read the rendered text back as a value of that type instead of leaving it a string: `duration: !!int "{{ .Meta.Duration }}"` emits `duration: 30`, not `duration: "30"`. The tag never reaches the document. When the text does not read as the declared type, rendering fails naming the front matter key. `!!str` and application tags (`!Ref`, …) are left alone. Until now everything a template touched came out quoted, and to the tools that parse front matter — Obsidian, dataview — those are different values.
- **`join` in the template FuncMap.** `{{ .Meta.Level | join ", " }}`, separator first, same argument order as Sprig's. Added for `!!seq` values, where a list has to be assembled into a flow sequence.
...
- **`TEMPLATES.md` layout in the "Синтаксис шаблонов" section.** The paragraph explaining that command metadata lives under `.Meta` sat between the two bullets of the syntax list, which ends the list at the paragraph and renders the YAML frontmatter bullet as a separate one-item list. The paragraph now follows the complete list. `TEMPLATES.md` is regenerated by `templates init` and overwritten by every `templates upgrade`, so the fix has to ship in the binary — a local edit to the file in a templates directory does not survive the next upgrade.
...
- **`changelog_lang` configuration key**, with the `SREKIT_CHANGELOG_LANG` environment form, so the language is set once rather than typed each time. Precedence is `--lang`, then the config value, then `en`. An unrecognized value from either source fails, naming the accepted ones, before any file is written — a typo does not silently fall back to English.
- **Language-variant artifact resolution.** A requested language tries `<name>.<lang>.yaml` across the whole source chain and, only if no source has it, falls back to `<name>.yaml` across the whole chain. Chain-first is the load-bearing half: given a customized `changelog.yaml` in a templates directory and no Russian variant, `--lang ru` yields the embedded Russian artifact rather than the user's English customization, because an English file — even their own — is not an answer to a request for Russian. Requesting a language for which no variant exists anywhere is indistinguishable from requesting none, not an error. Normalization stays idempotent, so `changelog.ru.yaml` passed as a name does not become `changelog.ru.yaml.yaml`.
- **`changelog release` and `changelog validate` read either vocabulary**, detected from the document's own first recognized change-type heading and never from `--lang` or the configuration: generation and parsing run in opposite directions, and a team that switches to Russian still has an English `CHANGELOG.md` from before the switch that a release must not corrupt. Placeholder pruning, link rewriting and every refusal work identically in both languages. A new `change-type-language` check reports which vocabulary was detected, so a user who expected one language and got the other sees the mismatch instead of a list of passing checks that measured the wrong thing. A document mixing both fails validation, naming the offending headings, and `release` refuses it and leaves the file byte-identical; a document with no recognized change types at all fails rather than being guessed at.
...
- **`--yanked` on `changelog release`** — marks a release withdrawn in the form Keep a Changelog specifies, `## [X.Y.Z] - YYYY-MM-DD [YANKED]`. Yanked releases were part of the format and appeared nowhere in `srekit`.
- **`srekit changelog validate [FILE]`** — a linter for the format, reporting six checks and exiting non-zero if any failed: `heading-shape` (`[X.Y.Z] - YYYY-MM-DD`, optionally ` [YANKED]`, which is what catches a regional date), `unreleased-section`, `version-order` (compared by numeric segment, so `1.10.0` sorts above `1.9.0`), `no-duplicate-versions`, `change-types` (the specification's six, whatever a customized artifact says), and `link-definitions`. Every check is reported whether it passed or failed — a person repairing a drifted changelog wants the full list in one pass — in the `OK` / `FAIL` style `postmortem --validate` already established. It reports; it does not repair, and it never writes.
...
[Unreleased]: https://github.com/jtprogru/srekit/compare/v0.32.1...HEAD
[0.32.1]: https://github.com/jtprogru/srekit/compare/v0.32.0...v0.32.1
