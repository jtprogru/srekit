...
- **Breaking — три генератора удалены: `capacity`, `retro` и `license` (вместе с алиасом `lic`).** Последний релиз, где они есть, — 0.29.3. `srekit` генерирует артефакты, которыми владеет дежурный или команда надёжности: ретро спринта — agile-церемония, capacity planning — работа в таблице, а LICENSE — разовый шаг настройки репозитория, унаследованный от команды `lic` из монолита gch. Имена не исчезают молча: скрытые заглушки в `cmd/retired.go` падают с ненулевым кодом и объяснением, называя релиз удаления и ссылку на migration-заметку, и не разбирают аргументы (`srekit retro` без `--team` сообщает об удалении, а не про пропущенный флаг). Заглушки будут убраны на 1.0. Что делать вместо: держать уже сгенерированный документ как статический шаблон, запинить 0.29.3, достать текст шаблона из `git show v0.29.3:internal/tmpl/templates/capacity.yaml`, а для LICENSE — взять license picker своего хостинга. Полный гайд — в [docs/migration/removed-commands.md](https://jtprogru.github.io/srekit/migration/removed-commands/).
- **Breaking — флаг `--template FILE` удалён совсем.** `license` была единственной командой, чей render-путь его читал; после её удаления флаг стал бы silently-ignored, а это в проекте считается дефектом. Теперь любая команда отвечает `Error: unknown flag: --template`. Кастомизация per-artifact — `<name>.yaml` в `templates_dir`.
- Встроенные артефакты `capacity.yaml` и `retro.yaml` больше не вкомпилированы. Пользовательские директории шаблонов не тронуты: оставшийся файл переклассифицируется из `customized` в `user-only` в `templates list`, а его снапшот собирается ближайшим `templates upgrade`.
- Внутреннее (не публичный контракт): вместе с флагом ушли legacy-ветка `text/template` в `render.buildBody` — её последний потребитель, — `render.WriteRaw`, `render.Options.TemplatePath`, всегда-истинное `render.Options.RenderArtifact`, `cliflags.Output.BindTemplateFlag` и `tmpl.ParseFile`. Формат v1-артефакта остался единственным путём рендера.
- Внутреннее: следом убран весь код, который держался на удалённой ветке. `render.Options.BootstrapJSON` и конверт, заворачивавший отрендеренный markdown в синтетический `{meta, sections:[{id:"body"}]}`, вместе с `render.extractH1`: конверт существовал для генератора без sections-манифеста, а такого не было с v0.20.0. `cliflags.Output.RenderOptionsStructured` схлопнут в `RenderOptions` — вся его работа сводилась к сбросу `BootstrapJSON`. `tmpl.Loader.Parse` возвращал распарсенный `text/template`, что нужно было только удалённой ветке; артефакты грузятся байтами через `LoadArtifactBytes` и парсятся в `internal/sections`. Публичный JSON-контракт `{meta, sections:[{id,title,type,body,required}]}` не изменился — его по-прежнему формируют сами генераторы.
