...
- every change-type subsection is one of `Added`, `Changed`, `Deprecated`, `Removed`, `Fixed`, `Security`
...
- **WHEN** a user runs `srekit changelog validate`
- **THEN** the change-type check SHALL be reported `FAIL` naming `Improvements` and listing the six allowed types
