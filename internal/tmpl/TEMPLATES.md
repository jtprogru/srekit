...
### Не-строковые значения frontmatter

Шаблон подставляет строку, поэтому по умолчанию значение так и уезжает в
документ строкой: `duration: "30"`. Инструментам, которые читают
frontmatter, это не всё равно. Повесь на значение явный YAML-тег, и
отрендеренный текст будет прочитан как значение этого типа:
...
Работают `!!int`, `!!float`, `!!bool`, `!!null`, `!!timestamp`, `!!seq`,
`!!map`. Тег — инструкция для рендера, в документ он не попадает. Если
текст не читается как заявленный тип (`!!int`, отрендеренный в `abc`),
рендер падает с ошибкой и именем ключа — молча строкой не станет.

`!!str` и прикладные теги (`!Ref`, …) не переинтерпретируются: явный
`!!str` — это и есть поведение по умолчанию, а чужой тег принадлежит тому
инструменту, который его читает.
...
| `join` | Склеить список строк через разделитель | `{{ .Meta.Level \| join ", " }}` |
...
### `tasker.yaml` — карточка задачи

| Поле | Тип | Описание |
|------|-----|----------|
| `.Meta.ID` | string | UUID |
| `.Meta.Now` | string | RFC3339 |
| `.Meta.Title` | string | Название задачи |
| `.Meta.Topic` | string | Тема, например `go` |
| `.Meta.Level` | []string | Уровни, например `middle`, `senior` |
| `.Meta.Format` | string | Формат ответа: `code`, `theory`, `design`, … |
| `.Meta.Duration` | int | Ожидаемое время решения в минутах |
