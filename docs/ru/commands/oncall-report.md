...
Сгенерировать **недельный отчёт дежурства** для команды — pages-таблица,
breakdown по toil, гигиена алертов. Период по умолчанию — текущая
неделя (Пн–Вс).
...
файла: `oncall-<slug-of-team>-<start>.md`.
...
Default-период начинается с понедельника и кончается воскресеньем.
Часы под капотом — `internal/clock.Now` (overridable для тестов;
production-бинарь использует `time.Now()`).

Если запустил отчёт в воскресенье 2026-05-10 — получишь
`2026-05-04 → 2026-05-10`. Регрессионный тест пинит это поведение.
...
- [`srekit ebp`](ebp.md) — политика бюджета ошибок, влияющая на
  нагрузку дежурного.
- [`srekit retro`](retro.md) — sprint-level ретро по on-call сигналам.
