...
Журнал релиз-за-релизом (что изменилось когда и почему) — [История миграции](v1-history.md).
...
## Рецепт обновления (v0.13.x → текущая)
...
#    template-выражения как есть — см. шаг переписывания ниже.
srekit templates migrate "$TPL"           # предпросмотр
...
- `header_body:` — свободный Markdown для всего, что не лезет в типизированные секции: blameless-врезки, свои цитаты-блоки, текст политики команды. (Для postmortem-style `.tmpl` файлов, которые заканчиваются `{{ range .Sections }}` loop'ом, конвертер корректно опускает header_body начиная с v0.26.0.)
...
ID секций — в `internal/tmpl/templates/<name>.yaml` в репо бинаря. Structured-контракт стабилен с v0.20.0.
...
Кастомизация отдельного артефакта в v1-модели — положить `<name>.yaml` в `templates_dir`. См. [Кастомные шаблоны](../guides/custom-templates.md) для полного цикла.
...
- [История миграции](v1-history.md) — журнал по релизам: что изменилось и когда.
...
- [Кастомные шаблоны](../guides/custom-templates.md) — steady-state цикл кастомизации.
