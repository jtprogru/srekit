...
For each step the destructive operations are isolated; you can stop after any step and re-run.

```bash
TPL=~/.srekit/templates   # or wherever your dir is

# 1. Bring in the v1 .yaml artifacts the binary now ships. Existing files
#    in your dir are not touched. Orphaned snapshots (e.g. for the old
#    postmortem.sections.yaml) are GC'd from .srekit-embedded/ automatically.
srekit templates upgrade "$TPL"

# 2. Convert any customized .tmpl in your dir to .yaml. Defaults to a
#    dry-run that prints the converted YAML; --apply writes files. The
#    converter copies template expressions verbatim — see the rewrite
#    step below.
srekit templates migrate "$TPL"           # preview
srekit templates migrate "$TPL" --apply   # write <name>.yaml next to each .tmpl

# 3. Rewrite data-shape references inside the generated YAML.
#    Every migrated generator moved its template root from .X to .Meta.X.
sed -E -i '' 's/\{\{ \.([A-Z][a-zA-Z]*)/\{\{ .Meta.\1/g' "$TPL"/*.yaml
# rfc-specific: function-call form not caught by the main regex
...
# 5. Once you trust the new YAML, delete the legacy files. They aren't
#    auto-deleted — they're your customizations.
...
- `header_body:` is the freeform-Markdown escape hatch — use it for blameless callouts, custom blockquotes, anything that doesn't fit the typed sections.
